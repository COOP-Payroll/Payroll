...
exports.getAllProjects = async (req, res, next) => {
  try {
    ...
    const data = projects.map((project) => {
      ...
      const totalAssignedEmployees = project.Positions.reduce(
        (total, position) => {
          return (
            total +
            (position.PositionProjectAssociation
              ? position.PositionProjectAssociation.noOfAssignedEmployees
              : 0)
          );
        },
        0
      ...
      totalRemainingEmployee =
        Number(project.numberOfEmployees) - Number(totalAssignedEmployees);
      totalAssignedPercent =
        (Number(totalAssignedEmployees) / Number(project.numberOfEmployees)) *
        100;
      return {
        ...projects1,
...
exports.getOneProject = async (req, res, next) => {
  try {
    ...
    if (!projects) {
      return next(
        createError.createError(404, "There is no projects with id " + id)
      );
    } else {
      res.status(200).json({
        success: true,
...
exports.assignProjectToEmployee = async (req, res, next) => {
  ...
  try {
    ...
    const getAllEmployeeUnderTheProject = await ProjectEmployee.count({
    ...
    const chechEmployeAssociation = await ProjectEmployee.findOne({
      where: {
        ProjectId: Number(projectId),
    ...
    if (chechEmployeAssociation) {
      ...
      return next(
        createError.createError(
          404,
          `Employee already  associated with the project`
        )
      );
    }

    const positionProjectAssociations =
      await PositionProjectAssociation.findOne({
        where: {
          ProjectId: Number(projectId),
    ...
    if (
      ...
      return next(
        createError.createError(
          400,
          `percent cannot exceeds  ${positionProjectAssociations.maximumPercentAllocation} %`
        )
      );
    }
    ...
    if (Number(employee.totalPercent) + Number(percent) <= 100) {
      ...
      if (
        parseFloat(checkAvailableBudget?.availableBudget) <
        grossValue * (Number(percent) / 100)
      ) {
        return next(
          createError.createError(
            400,
            " Employee budget exceeds available budget"
          )
        );
      }
      ...
      return next(
        createError.createError(
          400,
          `Total percent cannnot exceeds 100.currently you have reached  ${employee.totalPercent} percent  Cannot assign more projects.`
        )
      );
    }
  ...
  try {
    ...
    if (maximumPercentAllocation > 100) {
      return next(
        createError.createError(
          400,
          "maximumPercentAllocation can not be more than 100"
        )
      );
    }
    const projects = await Projects.findOne({
    ...
    if (
      positionIds.length !== noOfEmployees.length ||
      positionIds.length !== maximumPercentAllocation.length
    ) {
      return next(
        createError.createError(
          404,
          "No of employee ,maximumPercentAllocation or position mismatch"
        )
      );
    }
    ...
    const uniquePositionBudget = Array.from(new Set(budgets));
    const totalBudget = uniquePositionBudget.reduce(
      (total, budget) => total + parseFloat(budget),
      0
    );

    if (totalBudget > projects.availableBudget) {
      return next(
        createError.createError(400, "Budget it exceeds available budgets")
      );
    }
    ...
    const checkAllocatedNumberOfEmployee =
      await PositionProjectAssociation.findAll({
        where: { ProjectId: Number(projectId) },
      });
    ...
    if (
      Number(totalNoOfEmployeeForPosition) + Number(totalNoOfEmployees) >
      Number(projects.numberOfEmployees)
    ) {
      return next(
        createError.createError(
          400,
          `You can't assign the maximum allocation under this project is   ${projects.numberOfEmployees} employees currently ${totalNoOfEmployees} employees assigned to the project`
        )
      );
    }
...
exports.deassignPositionFromProject = async (req, res, next) => {
  ...
  try {
    ...
    const assignedPositions = await projects.getPositions({
      where: { id: Number(positionIds) },
    });
    ...
    if (assignedPositions.length !== positionIds.length) {
      return next(
        createError.createError(
          400,
          "One or more positions are not assigned to the project."
        )
      );
    }
      ...
      await ProjectPositionHistory.create(
        {
          ...
          noOfEmployees: Number(
            position.PositionProjectAssociation.noOfEmployees
          ),
          maximumPercentAllocation: Number(
            position.PositionProjectAssociation.maximumPercentAllocation
          ),
  ...
  try {
    ...
    if (employees.length !== employeeIds.length) {
      return next(
        createError.createError(
          404,
          "One or more employees is not associated to the position  "
        )
      );
    ...
    for (const employee of employees) {
      ...
      const positionProjectAssociation =
        await PositionProjectAssociation.findOne({
          where: {
            projectId: projectId,
      ...
      const currentNoOfEmployees =
        positionProjectAssociation.noOfEmployees || 0;
      const remainingEmployees =
        positionProjectAssociation.remainingEmployees || 0;
      const maxNoOfEmployees = 2;

      /* Specify the maximum number of employees for the position */ // Check if adding all employees would exceed the maximum number of employees for the project
      if (
...
exports.getAllUnassignedProjectForEmployee = async (req, res, next) => {
  try {
    ...
    const foundPosition = await Position.findOne({
    ...
    const employeePosition = await EmployeePosition.findOne({
      where: { PositionId: positionId, EmployeeId: employeeId, isActive: true },
    });
    ...
    if (!employeePosition) {
      return next(
        createError.createError(
          400,
          "Position is not associated to an employee"
        )
      );
    }
          ...
          through: {
            ...
            where: {
              ...
              [Op.and]: [
                {
                  noOfAssignedEmployees: {
                    [Op.lt]: Sequelize.col("noOfEmployees"),
                  },
                },
                { isActive: true },
              ],
    ...
    const filteredProjects = foundPosition1.map((position) => {
      const filteredProjectsForPosition = position.Projects.filter(
        (project) => {
          return project.Employees.length === 0;
        }
      );
  ...
  try {
    ...
    if (!positionProjectAssociation) {
      ...
      return next(
        createError.createError(
          404,
          "Position is not associated to the project"
        )
      );
    }
    ...
    await positionProjectAssociation.update(
      {
        noOfAssignedEmployees:
          Number(positionProjectAssociation.noOfAssignedEmployees) - 1,
      },
...
exports.updateProjectEmployeeAssocitation = async (req, res, next) => {
  ...
  try {
    ...
    if (
      ...
      const grossValue =
        (employee?.EmployeeInfos[0]?.grossEarning * percent) / 100;
