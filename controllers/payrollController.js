  ...
  try {
    ...
    const data = Employees.map((employee) => {
      ...
      const taxslab = taxslabs.find(
        (tax) =>
          totalTaxable > tax?.from_Salary && totalTaxable < tax?.to_Salary
      );
      if (taxslab) {
      ...
      grossSalary = (
        totalAllowance + employee.EmployeeInfos[0]?.basicSalary
      ).toFixed(2);
...
exports.getPayrollPerProject = async (req, res, next) => {
  try {
    ...
    if (!payrollDefinitionId) {
      return next(
        createError.createError(400, "payroll Definition id is required")
      );
    }
