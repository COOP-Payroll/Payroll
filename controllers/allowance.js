...
exports.getAllowanceById = async (req, res,next) => {
  try {
    ...
    if (!allowance) {
      ...
      return next(createError.createError(404,"There is no allowance by this ID"))
    } else {
...
exports.createAllowance = async (req, res, next) => {
  try {
    ...
    const grade = await Grade.findOne({
      where: { id: gradeId, CompanyId: req.user.id },
    });
...
exports.updateAllowance = async (req, res, next) => {
  try {
    ...
    const allowance= await Allowance.findOne({where:{ id, CompanyId: req.user.id}})
...
exports.deleteAllowance = async (req, res, next) => {
  try {
    const { id } = req.params;
    const allowance = await Allowance.findOne({ where: { id: id , CompanyId: req.user.id} });
