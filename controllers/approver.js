...
exports.getAllActiveApprovers = async (req, res, next) => {
  ...
  try {
    ...
    // res.status(503).json({ error: "Failed to retrieve Approvers" });
  }
};
...
exports.getApproverById = async (req, res, next) => {
  ...
  try {
    ...
    // res.status(503).json({ error: "Failed to retrieve Approver" });
  }
};
...
exports.createApprover = async (req, res, next) => {
  ...
  try {
    ...
    if (approvalMethod.approvalMethod === "hierarchy") {
      ...
      if (Number(level) < 1) {
        return next(
          createError.createError(400, "Level must be greater than zero")
        );
      }
    }
    ...
    if (approvalMethod === null) {
      ...
      if (ifMaster) {
        ...
        if (Number(masterApproverCount) >= 1 && req.body.isMaster === true) {
          ...
          return next(
            createError.createError(400, "Master approver is seeted already")
          );
        ...
        if (req.body.isMaster == true) {
          if (appMethod === "horizontal") {
            ...
            try {
              ...
              // res.status(503).json({ error: "Failed to save Approver" });
            }
          }
          ...
          if (appMethod === "horizontal") {
            ...
            if (appLevel < level) {
              await transaction.rollback();
              return next(
                createError.createError(400, `The maximum level is ${appLevel}`)
              );
            }
            if (level < 1) {
              await transaction.rollback();
      ...
      //if no master approval
        ...
        if (req.body.isMaster === true) {
          return next(
            createError.createError(400, "Cannot set as master approver")
          );
        }
        if (appMethod === "horizontal") {
          //count saved approver for this company except for master
          try {
            if (minimumApp === null) {
              ...
              return res.status(200).json({
                ...
                message:
                  "Approver setted successfully you can approve your payroll",
              });
          ...
          if (level < 1) {
            await transaction.rollback();
            return next(createError.createError(400, `The minimum level is 1`));
          }
    ...
    // res.status(503).json({ error: "Failed to create Approver" });
  }
};
...
exports.updateApprover = async (req, res, next) => {
  ...
  try {
    ...
    // res.status(503).json({ error: "Failed to update Approver" });
  }
};
...
exports.deleteApprover = async (req, res, next) => {
  ...
  try {
    ...
    // res.status(503).json({ error: "Failed to delete Approver" });
  }
};
